...
def extract_all_configurations_density_data(all_config_data, save_path="design_space_analysis"):
    """提取全部构型的三个关键性能数据和激活面信息，分成5个CSV文件每个保存20万条数据"""
    print(f"\n=== 提取全部{len(all_config_data)}个构型的关键性能数据和激活面信息 ===")

    # 提取所有构型的三个关键性能数据、激活面信息并计算E1, E2, E3
    data = []
    for i, config in enumerate(all_config_data):
        normalized_E1 = config['normalized_E1']
        normalized_E2 = config['normalized_E2']  # 新增E2
        normalized_E3 = config['normalized_E3']  # 新增E3
        relative_density = config['relative_density']
        E1 = normalized_E1 * relative_density  # 计算 E1 = normalized_E1 × relative_density
        E2 = normalized_E2 * relative_density  # 新增E2计算
        E3 = normalized_E3 * relative_density  # 新增E3计算
        active_faces = sorted([face_id + 1 for face_id in config['active_faces']])  # 转换为1-20的面ID并排序
        active_faces_str = ';'.join(map(str, active_faces))  # 用分号分隔的面ID字符串

        data.append({
            'index': i,
            'normalized_E1': normalized_E1,
            'normalized_E2': normalized_E2,  # 新增E2
            'normalized_E3': normalized_E3,  # 新增E3
            'relative_density': relative_density,
            'E1': E1,  # 修改字段名从E改为E1
            'E2': E2,  # 新增E2
            'E3': E3,  # 新增E3
            'anisotropy': config['anisotropy'],  # 新增各向异性指数
            'active_faces': active_faces_str  # 激活的面ID列表
        })
    ...
    num_files = (total_configs + chunk_size - 1) // chunk_size  # 向上取整

    print(f"总构型数: {total_configs}")
    print(f"每个文件保存: {chunk_size} 条数据")
    print(f"文件数量: {num_files}")
    print(f"保存的字段: index, normalized_E1, normalized_E2, normalized_E3, relative_density, E1, E2, E3, anisotropy, active_faces")

    saved_files = []
    for i in range(num_files):
        start_idx = i * chunk_size
        end_idx = min((i + 1) * chunk_size, total_configs)

        chunk_df = df.iloc[start_idx:end_idx]

        # 保存为CSV和Excel文件
        csv_path = f"{save_path}/all_configs_key_data_part_{i + 1}.csv"
        excel_path = f"{save_path}/all_configs_key_data_part_{i + 1}.xlsx"
        ...
        saved_files.append({
            ...
            'num_configs': len(chunk_df),
            'start_index': start_idx,
            'end_index': end_idx - 1
        })

        print(f"文件 {i + 1}: 保存了 {len(chunk_df)} 条数据 (索引 {start_idx}-{end_idx - 1})")
        print(f"  CSV文件: {csv_path}")
        print(f"  Excel文件: {excel_path}")

    # 保存文件信息汇总
    summary_file = f"{save_path}/all_configs_key_data_summary.txt"
    with open(summary_file, "w") as f:
        f.write("全部构型关键性能数据和激活面信息文件汇总\n")
        f.write("====================================\n\n")
        f.write(f"总构型数: {total_configs}\n")
        f.write(f"文件数量: {num_files}\n")
        f.write(f"每个文件数据量: {chunk_size} 条 (最后一个文件可能少于这个数量)\n")
        f.write(f"保存的字段: index, normalized_E1, normalized_E2, normalized_E3, relative_density, E1, E2, E3, anisotropy, active_faces\n")
        f.write(f"E1计算方式: E1 = normalized_E1 × relative_density\n")
        f.write(f"E2计算方式: E2 = normalized_E2 × relative_density\n")
        f.write(f"E3计算方式: E3 = normalized_E3 × relative_density\n")
        f.write(f"active_faces格式: 用分号分隔的面ID，例如 '1;3;5;7'\n\n")

        f.write("文件详细信息:\n")
        for file_info in saved_files:
            f.write(f"文件 {file_info['part']}:\n")
            f.write(f"  数据条数: {file_info['num_configs']}\n")
            f.write(f"  索引范围: {file_info['start_index']}-{file_info['end_index']}\n")
            f.write(f"  CSV路径: {file_info['csv_path']}\n")
            f.write(f"  Excel路径: {file_info['excel_path']}\n\n")

        # 总体统计信息
        f.write("总体数据统计:\n")
        f.write(f"E1范围: {df['normalized_E1'].min():.3f} - {df['normalized_E1'].max():.3f}\n")
        f.write(f"E2范围: {df['normalized_E2'].min():.3f} - {df['normalized_E2'].max():.3f}\n")
        f.write(f"E3范围: {df['normalized_E3'].min():.3f} - {df['normalized_E3'].max():.3f}\n")
        f.write(f"相对密度范围: {df['relative_density'].min():.3f} - {df['relative_density'].max():.3f}\n")
        f.write(f"E1范围: {df['E1'].min():.3f} - {df['E1'].max():.3f}\n")
        f.write(f"E2范围: {df['E2'].min():.3f} - {df['E2'].max():.3f}\n")
        f.write(f"E3范围: {df['E3'].min():.3f} - {df['E3'].max():.3f}\n")
        f.write(f"各向异性范围: {df['anisotropy'].min():.3f} - {df['anisotropy'].max():.3f}\n")
        f.write(f"平均E1: {df['normalized_E1'].mean():.3f}\n")
        f.write(f"平均E2: {df['normalized_E2'].mean():.3f}\n")
        f.write(f"平均E3: {df['normalized_E3'].mean():.3f}\n")
        f.write(f"平均相对密度: {df['relative_density'].mean():.3f}\n")
        f.write(f"平均E1: {df['E1'].mean():.3f}\n")
        f.write(f"平均E2: {df['E2'].mean():.3f}\n")
        f.write(f"平均E3: {df['E3'].mean():.3f}\n")
        f.write(f"平均各向异性: {df['anisotropy'].mean():.3f}\n\n")

        # 相关性分析
        corr_e1_density = df['normalized_E1'].corr(df['relative_density'])
        corr_e2_density = df['normalized_E2'].corr(df['relative_density'])
        corr_e3_density = df['normalized_E3'].corr(df['relative_density'])
        corr_e1_e2 = df['normalized_E1'].corr(df['normalized_E2'])
        corr_e1_e3 = df['normalized_E1'].corr(df['normalized_E3'])
        corr_e2_e3 = df['normalized_E2'].corr(df['normalized_E3'])

        f.write("相关性分析:\n")
        f.write(f"E1与相对密度相关系数: {corr_e1_density:.3f}\n")
        f.write(f"E2与相对密度相关系数: {corr_e2_density:.3f}\n")
        f.write(f"E3与相对密度相关系数: {corr_e3_density:.3f}\n")
        f.write(f"E1与E2相关系数: {corr_e1_e2:.3f}\n")
        f.write(f"E1与E3相关系数: {corr_e1_e3:.3f}\n")
        f.write(f"E2与E3相关系数: {corr_e2_e3:.3f}\n\n")

        # 找出各方向E值最高的前10个构型
        top_e1_configs = df.nlargest(10, 'E1')
        top_e2_configs = df.nlargest(10, 'E2')
        top_e3_configs = df.nlargest(10, 'E3')

        f.write("E1值最高的前10个构型:\n")
        for idx, row in top_e1_configs.iterrows():
            f.write(
                f"  索引 {row['index']}: E1={row['normalized_E1']:.3f}, 相对密度={row['relative_density']:.3f}, E1={row['E1']:.3f}, 激活面={row['active_faces']}\n")

        f.write("\nE2值最高的前10个构型:\n")
        for idx, row in top_e2_configs.iterrows():
            f.write(
                f"  索引 {row['index']}: E2={row['normalized_E2']:.3f}, 相对密度={row['relative_density']:.3f}, E2={row['E2']:.3f}, 激活面={row['active_faces']}\n")

        f.write("\nE3值最高的前10个构型:\n")
        for idx, row in top_e3_configs.iterrows():
            f.write(
                f"  索引 {row['index']}: E3={row['normalized_E3']:.3f}, 相对密度={row['relative_density']:.3f}, E3={row['E3']:.3f}, 激活面={row['active_faces']}\n")

        # 分析激活面模式
        f.write("\n激活面模式分析:\n")
        # 统计每个面的激活频率
        face_activation_count = {face_id: 0 for face_id in range(1, 21)}
        for faces_str in df['active_faces']:
            faces = [int(face) for face in faces_str.split(';')] if faces_str else []
            for face in faces:
                face_activation_count[face] += 1

        f.write("各面激活频率:\n")
        for face_id in range(1, 21):
            freq = face_activation_count[face_id] / total_configs
            f.write(f"  面{face_id}: {freq:.3f}\n")

        # 找出最常激活的面
        top_faces = sorted(face_activation_count.items(), key=lambda x: x[1], reverse=True)[:5]
        f.write(f"\n最常激活的前5个面:\n")
        for face_id, count in top_faces:
            freq = count / total_configs
            f.write(f"  面{face_id}: {freq:.3f} ({count}次)\n")

    print(f"\n数据保存完成!")
    print(f"文件汇总信息已保存至: {summary_file}")
    print(f"关键性能数据统计:")
    print(f"  E1范围: {df['normalized_E1'].min():.3f} - {df['normalized_E1'].max():.3f}")
    print(f"  E2范围: {df['normalized_E2'].min():.3f} - {df['normalized_E2'].max():.3f}")
    print(f"  E3范围: {df['normalized_E3'].min():.3f} - {df['normalized_E3'].max():.3f}")
    print(f"  相对密度范围: {df['relative_density'].min():.3f} - {df['relative_density'].max():.3f}")
    print(f"  平均E1: {df['E1'].mean():.3f}, 平均E2: {df['E2'].mean():.3f}, 平均E3: {df['E3'].mean():.3f}")
    print(f"  E1与相对密度相关性: {corr_e1_density:.3f}")
    print(f"  E2与相对密度相关性: {corr_e2_density:.3f}")
    print(f"  E3与相对密度相关性: {corr_e3_density:.3f}")
...
def visualize_top_e1_network_analysis(analysis_results, top_k=1000,
    ...
    for i in range(20):
        face_id = i + 1
        G.add_node(face_id, activation_freq=face_activation_freq[i])

    # 添加边 - 只添加共现频率较高的边
    edge_threshold = 0.1  # 只显示共现频率大于10%的边
    ...
    # 节点大小基于激活频率
    node_sizes = [face_activation_freq[i] * 5000 + 500 for i in range(20)]

    # 节点颜色基于激活频率
    node_colors = [face_activation_freq[i] for i in range(20)]
    ...
    # 添加颜色条
    plt.colorbar(nodes, label='面激活频率')

    # 设置标题
    plt.title(f'前{top_k}个高E1构型的面激活网络分析\n'
              f'节点大小表示激活频率，边粗细表示共现频率',
              fontsize=16, fontweight='bold', pad=20)

    # 添加统计信息
    stats_text = (f"统计信息:\n"
                  f"• 平均激活面数: {analysis_results['avg_faces']:.2f}\n"
                  f"• 核心面: {[face for face, freq in analysis_results['face_freq_sorted'][:5] if freq > 0.5]}\n"
                  f"• 总边数: {len(G.edges())}")
    ...
    plt.axis('off')
    plt.tight_layout()
    plt.savefig(f"{save_path}/top_{top_k}_e1_network.png", dpi=300, bbox_inches='tight')
    plt.close()

    print(f"高E1构型网络分析图已保存至: {save_path}/top_{top_k}_e1_network.png")

    # 保存详细数据
    with open(f"{save_path}/top_{top_k}_e1_analysis.txt", "w") as f:
        f.write(f"前{top_k}个高E1构型详细分析报告\n")
        f.write("=" * 50 + "\n\n")

        f.write("面激活频率统计:\n")
        for face_id, freq in analysis_results['face_freq_sorted']:
            f.write(f"面 {face_id}: {freq:.3f}\n")

        f.write(f"\n平均激活面数: {analysis_results['avg_faces']:.2f}\n\n")

        f.write("重要的面共现关系:\n")
        for (face1, face2), count in cooccurrence_pairs[:30]:
            freq = count / top_k
            f.write(f"面 {face1}-面 {face2}: {freq:.3f} ({count}次)\n")
...
def visualize_design_space(all_config_data, analysis_results, save_path="design_space_analysis"):
    ...
    # 使用顶刊配色方案
    color_e1 = '#add3e2'  # 清新的蓝绿色 - E1
    color_e2 = '#efb696'  # 柔和的灰绿色 - E2
    color_e3 = '#c2bdde'  # 淡雅的紫色 - E3
    color_ani = '#82afda' # 自然绿色 - 各向异性
    ...
    scatter = plt.scatter(e1_vals, ani_vals, c=num_faces,
                          cmap='viridis', alpha=0.6, s=20)

    plt.xlabel('Normalized E1')
    plt.ylabel('Anisotropy Index')
    plt.title('Design Space: E1 vs Anisotropy (Colored by Number of Faces)')

    cbar = plt.colorbar(scatter)
    cbar.set_label('Number of Faces')

    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(f"{save_path}/e1_vs_anisotropy.png", dpi=300, bbox_inches='tight')
    plt.close()
    ...
    bars_e1 = ax1.bar(x_pos - bar_width, importance_e1, bar_width,
                     color=color_e1, alpha=0.8, label='E1 Importance')
    bars_e2 = ax1.bar(x_pos, importance_e2, bar_width,
                     color=color_e2, alpha=0.8, label='E2 Importance')
    bars_e3 = ax1.bar(x_pos + bar_width, importance_e3, bar_width,
                     color=color_e3, alpha=0.8, label='E3 Importance')

    ax1.set_xlabel('Face ID', fontsize=12)
    ax1.set_ylabel('Feature Importance', fontsize=12)
    ax1.set_title('Face Importance for Directional Stiffness (E1, E2, E3)', fontsize=14)
    ax1.set_xticks(x_pos)
    ax1.set_xticklabels([str(i) for i in range(1, 21)])
    ax1.legend(fontsize=10)
    ax1.grid(True, alpha=0.3, axis='y')

    # 第二张图：各向异性重要性
    bars_ani = ax2.bar(x_pos, importance_ani, color=color_ani, alpha=0.8)
    ax2.set_xlabel('Face ID', fontsize=12)
    ax2.set_ylabel('Feature Importance', fontsize=12)
    ax2.set_title('Face Importance for Anisotropy Index', fontsize=14)
    ax2.set_xticks(x_pos)
    ax2.set_xticklabels([str(i) for i in range(1, 21)])
    ax2.grid(True, alpha=0.3, axis='y')

    plt.tight_layout()
    plt.savefig(f"{save_path}/face_importance_combined.png", dpi=300, bbox_inches='tight')
    plt.close()
    ...
    # E1分布
    ax1.hist(e1_vals, bins=50, alpha=0.7, color=color_e1)
    ax1.set_title('E1 Distribution')
    ax1.set_xlabel('Normalized E1')
    ax1.set_ylabel('Frequency')

    # 各向异性分布
    ax2.hist(ani_vals, bins=50, alpha=0.7, color=color_ani)
    ax2.set_title('Anisotropy Distribution')
    ax2.set_xlabel('Anisotropy Index')
    ax2.set_ylabel('Frequency')

    plt.tight_layout()
    plt.savefig(f"{save_path}/performance_distribution.png", dpi=300, bbox_inches='tight')
    plt.close()
...
def main():
    ...
    # 打印CSV的列名
    print("CSV columns:", dataset.df_csv.columns.tolist())
    # 加载模型
    ...
    # 修改：提取全部100万个构型的关键性能数据和激活面信息，分成5个CSV文件
    print("\n=== 开始保存全部构型关键性能数据和激活面信息到CSV文件 ===")
    saved_files = extract_all_configurations_density_data(all_configs)
    ...
    with open("design_space_analysis/detailed_analysis_report.txt", "w") as f:
        ...
        # 数据文件信息
        f.write("1. 数据文件信息:\n")
        f.write(f"   总构型数: {len(all_configs)}\n")
        f.write(f"   数据文件数量: {len(saved_files)}\n")
        f.write("   文件列表:\n")
        for file_info in saved_files:
            f.write(
                f"     文件 {file_info['part']}: {file_info['num_configs']} 条数据, 索引 {file_info['start_index']}-{file_info['end_index']}\n")
            f.write(f"       CSV: {file_info['csv_path']}\n")
            f.write(f"       Excel: {file_info['excel_path']}\n")
        f.write("\n")

        # 性能统计
        f.write("2. 关键性能统计:\n")
        e1_vals = [c['normalized_E1'] for c in all_configs]
        density_vals = [c['relative_density'] for c in all_configs]
        e_vals = [e1 * density for e1, density in zip(e1_vals, density_vals)]

        f.write(f"   E1范围: {min(e1_vals):.3f} - {max(e1_vals):.3f}\n")
        f.write(f"   相对密度范围: {min(density_vals):.3f} - {max(density_vals):.3f}\n")
        f.write(f"   E范围: {min(e_vals):.3f} - {max(e_vals):.3f}\n")
        f.write(f"   平均E1: {np.mean(e1_vals):.3f}\n")
        f.write(f"   平均相对密度: {np.mean(density_vals):.3f}\n")
        f.write(f"   平均E: {np.mean(e_vals):.3f}\n")
        f.write("\n")
        ...
        if high_e1_stiffness_clusters:
            ...
            for cluster_id, result in high_e1_stiffness_clusters.items():
                ...
                f.write(f"       平均E1: {result['avg_E1']:.3f}, 平均相对密度: {result['avg_relative_density']:.3f}\n")
                key_faces = [i + 1 for i, freq in enumerate(result['face_activation']) if freq > 0.7]
        ...
        if high_e2_stiffness_clusters:
            ...
            for cluster_id, result in high_e2_stiffness_clusters.items():
                f.write(f"     模式 {cluster_id + 1} (样本数: {result['size']}):\n")
                f.write(f"       平均E2: {result['avg_E2']:.3f}, 平均相对密度: {result['avg_relative_density']:.3f}\n")
                key_faces = [i + 1 for i, freq in enumerate(result['face_activation']) if freq > 0.7]
                f.write(f"       关键激活面: {sorted(key_faces)}\n")
        ...
        if high_e3_stiffness_clusters:
            ...
            for cluster_id, result in high_e3_stiffness_clusters.items():
                f.write(f"     模式 {cluster_id + 1} (样本数: {result['size']}):\n")
                f.write(f"       平均E3: {result['avg_E3']:.3f}, 平均相对密度: {result['avg_relative_density']:.3f}\n")
                key_faces = [i + 1 for i, freq in enumerate(result['face_activation']) if freq > 0.7]
                f.write(f"       关键激活面: {sorted(key_faces)}\n")
    ...
    print("分析完成! 所有结果已保存至 design_space_analysis/ 目录")
