...
ORTHOGONAL_DIAGONAL_GROUPS = {
    'E1_Group': [8, 10],  # 对E1最重要的组 (X轴)
    'E2_Group': [11, 13],  # 对E2最重要的组 (Y轴)
    'E3_Group': [5, 19]  # 对E3最重要的组 (Z轴)
}
...
def analyze_face_geometry_and_importance(dataset, importance_e1, importance_e2, importance_e3, importance_ani,
    ...
    with open(f"{save_path}/face_geometry_analysis_report.txt", "w") as f:
        ...
        f.write("\n结论:\n")
        f.write("面的重要性并非单纯由连接数量决定，而是由其空间方位（法向量方向）与载荷方向的耦合关系决定。\n")
        f.write("面8和10之所以对E1重要，是因为它们的法向量在X轴上的投影分量最大，能最有效地抵抗X方向载荷。\n")
        f.write("面11和13之所以对E2重要，是因为它们的法向量在Y轴上的投影分量最大，能最有效地抵抗Y方向载荷。\n")
        f.write("面5和19之所以对E3重要，是因为它们的法向量在Z轴上的投影分量最大，能最有效地抵抗Z方向载荷。\n")
        f.write("这一发现证明了GPGA-VAE模型能够超越表观对称性，精准捕捉到微观几何特征与宏观性能之间的内在物理联系。\n")
...
def analyze_force_transmission_paths(all_config_data, dataset, save_path="design_space_analysis/force_path_analysis"):
    ...
    with open(f"{save_path}/force_path_analysis_report.txt", "w") as f:
        ...
        f.write("\n分析:\n")
        f.write("面8和10与X轴的夹角较小，能更直接地抵抗X方向载荷，因此对E1最重要。\n")
        f.write("面11和13与Y轴的夹角较小，能更直接地抵抗Y方向载荷，因此对E2最重要。\n")
        f.write("面5和19与Z轴的夹角较小，能更直接地抵抗Z方向载荷，因此对E3最重要。\n")
        f.write("它们常与法向量方向各异的面共现，形成高效的力传递网络：\n")
        f.write("- 主承载面负责直接抵抗特定方向载荷\n")
        f.write("- 其他面负责将力分散到整个结构\n")
        f.write("这种分工合作解释了为什么这些面对特定方向的刚度如此重要。\n")
...
def main():
    ...
    with open("design_space_analysis/detailed_analysis_report.txt", "w") as f:
        ...
        # 物理机制分析
        f.write("6. 物理机制分析:\n")
        f.write("   面的重要性并非由其连接数量决定，而是由其空间方位（法向量方向）与载荷方向的耦合关系决定。\n")
        f.write("   面8和10之所以对E1重要，是因为它们的法向量在X轴上的投影分量最大，能最有效地抵抗X方向载荷。\n")
        f.write("   面11和13之所以对E2重要，是因为它们的法向量在Y轴上的投影分量最大，能最有效地抵抗Y方向载荷。\n")
        f.write("   面5和19之所以对E3重要，是因为它们的法向量在Z轴上的投影分量最大，能最有效地抵抗Z方向载荷。\n")
        f.write("   它们常与法向量方向各异的面共现，形成高效的力传递网络：\n")
        ...
        # 设计建议
        f.write("7. 设计建议:\n")
        f.write("   - 高刚度设计存在多种模式，而非单一最优配置\n")
        f.write("   - 甜蜜点设计（中等刚度且低各向异性）具有特定的面激活模式\n")
        f.write("   - 面8和面10是创造高E1刚度的关键面，应优先考虑激活\n")
        f.write("   - 面11和面13是创造高E2刚度的关键面，应优先考虑激活\n")
        f.write("   - 面5和面19是创造高E3刚度的关键面，应优先考虑激活\n")
        f.write("   - 刚度与各向异性之间存在负相关关系，但可通过复杂拓扑设计部分突破\n")
