...
def visualize_face_in_cube(dataset, face_ids, save_path="design_space_analysis10.30/face_visualization"):
    """可视化指定面在立方体中的位置和方向"""
...
def analyze_face_connectivity(dataset, save_path="design_space_analysis10.30/face_connectivity"):
    """分析面连接关系"""
    os.makedirs(save_path, exist_ok=True)
...
def extract_face_designs(all_config_data, face_ids, save_path="design_space_analysis10.30/face_designs"):
    """提取指定面激活时的典型构型"""
    os.makedirs(save_path, exist_ok=True)
...
def plot_face_activation_vs_performance(all_config_data, face_ids, stiffness_type='E1',
                                        save_path="design_space_analysis10.30/face_activation_analysis"):
    """构建指定面的激活概率与性能的关系图"""
    os.makedirs(save_path, exist_ok=True)
...
def visualize_top_e1_network_analysis(analysis_results, top_k=1000,
                                      save_path="design_space_analysis10.30/top_e1_network"):
    """改进版：在复杂网络中揭示隐藏规律的可视化"""
    import os
    os.makedirs(save_path, exist_ok=True)
    ...
    for i in range(20):
        face_id = i + 1
        if face_activation_freq[i] > 0.05:  # 只显示激活频率>5%的面
    ...
    for (face1, face2), count in cooccurrence_pairs:
        freq = count / top_k
        # 提高阈值，只显示强连接
        if freq > 0.15 and face1 in G.nodes() and face2 in G.nodes():
    ...
    # 1. 节点大小基于激活频率（对数缩放以增强差异）
    node_sizes = []
    for node in G.nodes():
        freq = face_activation_freq[node - 1]
        # 使用对数缩放增强大小差异
        scaled_size = 800 + 4000 * (np.log(freq + 0.01) - np.log(0.01)) / (np.log(1.01) - np.log(0.01))
        node_sizes.append(max(500, min(3000, scaled_size)))

    # 2. 节点颜色统一基于激活频率（使用连续颜色条）
    node_frequencies = [face_activation_freq[node - 1] for node in G.nodes()]

    # 使用热力图颜色映射
    norm = Normalize(vmin=min(node_frequencies), vmax=max(node_frequencies))
    cmap = cm.get_cmap('viridis')  # 或者 'viridis', 'inferno'
    node_colors = [cmap(norm(freq)) for freq in node_frequencies]
    ...
    for u, v in G.edges():
        freq = G[u][v]['weight']
        # 边的宽度基于共现频率
        edge_width = 1 + 15 * freq  # 基础宽度1，最大宽度16
    ...
    # 特别调整核心面的位置
    core_faces = [8, 10]  # E1核心面
    if all(face in G.nodes() for face in core_faces):
        # 将核心面放在中心偏上位置
        center = np.array([0, 0])
        pos[8] = center + np.array([-0.8, 0.3])
        pos[10] = center + np.array([0.8, 0.3])
        # 重新计算布局，固定核心面
        pos = nx.spring_layout(G, pos=pos, fixed=core_faces, k=2, iterations=100)

    # === 绘制网络 ===
    # 1. 先绘制边（灰色）
    nx.draw_networkx_edges(G, pos,
                           width=edge_widths,
                           edge_color='#666666',  # 统一灰色
                           alpha=0.6,
                           ax=ax)
    ...
    cbar.set_label('Face Activation Frequency', fontsize=12, fontweight='bold')
    cbar.ax.tick_params(labelsize=10)

    # === 设置标题和样式 ===
    ax.set_title(f'Top {top_k} High-E1 Configurations: Face Co-occurrence Network\n'
                 f'● Node size/color = Activation frequency | Edge thickness = Co-occurrence frequency',
                 fontsize=16, fontweight='bold', pad=20)
    ax.axis('off')

    # === 添加统计信息 ===
    stats_text = (f"Network Statistics:\n"
                  f"• Nodes: {len(G.nodes())} (faces with freq > 5%)\n"
                  f"• Edges: {len(G.edges())} (co-occurrence > 15%)\n"
                  f"• Avg. active faces: {analysis_results['avg_faces']:.2f}\n"
                  f"• Core connection (F8-F10): {next((freq for (f1, f2), freq in significant_edges if set([f1, f2]) == {8, 10}), 0):.3f}")
    ...
    # === 突出显示核心发现 ===
    if any(set([8, 10]) == set(edge) for edge in G.edges()):
        discovery_text = ("Key Pattern:\n"
                          "F8 & F10 form the\n"
                          "essential 45° diagonal\n"
                          "support core")

        ax.text(0.98, 0.98, discovery_text, transform=ax.transAxes, fontsize=12,
                bbox=dict(boxstyle="round,pad=0.5", facecolor="lightgreen", alpha=0.9),
                verticalalignment='top', horizontalalignment='right',
                fontweight='bold')

    plt.tight_layout()
    plt.savefig(f"{save_path}/top_{top_k}_e1_network_improved.png",
                dpi=300, bbox_inches='tight', facecolor='white')
    plt.close()

    print(f"改进版网络分析图已保存至: {save_path}/top_{top_k}_e1_network_improved.png")

    return G
def analyze_face_geometry_and_importance(dataset, importance_e1, importance_e2, importance_e3, importance_ani,
                                         save_path="design_space_analysis10.30/face_geometry_analysis"):
    """分析面的几何属性与其重要性之间的关系"""
    os.makedirs(save_path, exist_ok=True)
...
def analyze_force_transmission_paths(all_config_data, dataset, save_path="design_space_analysis10.30/force_path_analysis"):
...
def visualize_design_space(all_config_data, analysis_results, save_path="design_space_analysis10.30"):
    """可视化设计空间和分析结果"""
    print("\n=== 生成设计空间可视化 ===")
...
def main():
    ...
    # 创建输出目录 - 确保先创建主目录
    os.makedirs("design_space_analysis10.30", exist_ok=True)
    os.makedirs("design_space_analysis10.30/detailed_analysis", exist_ok=True)
    ...
    # 保存所有生成的数据 - 确保目录存在
    os.makedirs("design_space_analysis10.30", exist_ok=True)
    np.save("design_space_analysis10.30/all_configs.npy", all_configs)
    ...
    # 输出详细分析报告
    with open("design_space_analysis10.30/detailed_analysis_report.txt", "w") as f:
        f.write("=== 超材料设计空间详细分析报告 ===\n\n")
        f.write(f"基于 {len(all_configs)} 个生成构型的分析结果\n\n")

        f.write("🌟 核心科学发现：旋转对称性与统一设计原则 🌟\n")
        f.write("=============================================\n\n")

        f.write("关键发现：\n")
        f.write("1. 结构表现出完美的正交各向异性特征\n")
        f.write("2. 三个主方向遵循相同的设计原则\n")
        f.write("3. 45°对角面构成核心支撑结构\n")
        f.write("4. 设计具有旋转对称性\n\n")

        f.write("统一设计原则：\n")
        f.write("- X方向(E1): 面8和10形成45°对角支撑\n")
        f.write("- Y方向(E2): 面4和13形成45°对角支撑\n")
        f.write("- Z方向(E3): 面3和16形成45°对角支撑\n")
        f.write("这三个面对在各自方向上提供主要刚度支撑，体现了统一的设计机理。\n\n")
        ...
        f.write("   核心面对及其重要性:\n")
        f.write("   X方向(E1): 面8和10 - 构成45°对角支撑核心\n")
        f.write(f"     面8重要性: {importance_e1[7]:.4f}, 面10重要性: {importance_e1[9]:.4f}\n")
        f.write("   Y方向(E2): 面4和13 - 构成45°对角支撑核心\n")
        f.write(f"     面4重要性: {importance_e2[3]:.4f}, 面13重要性: {importance_e2[12]:.4f}\n")
        f.write("   Z方向(E3): 面3和16 - 构成45°对角支撑核心\n")
        f.write(f"     面3重要性: {importance_e3[2]:.4f}, 面16重要性: {importance_e3[15]:.4f}\n")
        f.write("\n")
        ...
        # 总结
        f.write("=== 总结 ===\n")
        f.write("本研究通过大规模设计空间探索，揭示了超材料结构的统一设计原则：\n")
        f.write("• 三个主方向均依赖45°对角面提供主要刚度支撑\n")
        f.write("• 结构表现出完美的旋转对称性特征\n")
        f.write("• 正交各向异性源于统一的几何构型原则\n")
        f.write("这一发现为智能超材料设计提供了重要的理论指导。\n")
    ...
    print("分析完成! 所有结果已保存至 design_space_analysis10.30/ 目录")
