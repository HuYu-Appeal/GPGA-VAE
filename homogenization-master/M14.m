...
% ========== 用户自定义参数 ==========
h5File = 'voxel_models_batch_14.h5'; % HDF5文件路径
outputCSV = 'homogenized_results_14new.csv'; % 输出CSV文件名
...
startModelIndex = 16; % 设置起始模型索引

if useParallel
resultsCell = cell(numModels, 1);
parfor i = startModelIndex:numModels
key = keys{i};
...
for i = startModelIndex:numModels
key = keys{i};
fprintf('Processing model: %s\n', key);
